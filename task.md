# task.md — Deep Reinforcement Learning Snake Agent
...
| M1-02 | Snake game core | Implement game/snake.py: 16×16 grid, snake body list, food placement, collision detection (wall + self), reset(), play_step(action) returns (reward, done, score). Rewards: +10 eat, -10 die, +1 closer, -1 farther. | [backend] | M1-01 | ✅ | Implemented SnakeGame class with correct 16x16 grid and RL rewards. Fixed index out of bounds in sensors and state builder |
...
| M3-02 | Hero section | Build hero: full-width dark section, animated snake canvas background (snake crawls across grid using JS setInterval), title "Deep Reinforcement Learning Snake Agent", subtitle, tech chips (PyTorch, DQN, 27-input, FastAPI, WebSocket, BFS, A*, Minimax), scroll CTA arrow. | [frontend] | M3-01 | ✅ | "Hero rebuilt: video bg + GSAP scroll-pinned text reveal, 6-line sequence, scroll unlock to algo sections" |
...
| M4-05 | Fix bugs from testing | Address any crashes, rendering glitches, WS errors, or chart issues found in M4-01 through M4-04. | [backend] [frontend] | M4-02, M4-03, M4-04 | ✅ | "DQN fixed: epsilon lifetime persistence, reward simplified to 0/+10/-10, BATCH 256, algorithm-leads hybrid arbitration. Fixed _custom_save missing lifetime_games — root cause of epsilon reset bug" |
