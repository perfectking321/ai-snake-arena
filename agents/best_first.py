...
def solve(snake, food, grid_size):
    ...
    while pq:
        h_val, curr, path = heapq.heappop(pq)
        ...
        for dx_step, dy_step in [(0,-1), (0,1), (-1,0), (1,0)]:
            ...
            if 0 <= nxt[0] < grid_size and 0 <= nxt[1] < grid_size:
                if nxt not in visited and nxt not in obstacles:
                    nxt_h = h_func(nxt, goal)
                    heapq.heappush(pq, (nxt_h, nxt, path + [nxt]))
                    
    action = [1,0,0] 
    path_coords = []
    current_h = h_func(start, goal)
    ...
    return {
        ...
        "path": path_coords,
        "algo_stat": {"label": "h", "value": current_h}
    }
